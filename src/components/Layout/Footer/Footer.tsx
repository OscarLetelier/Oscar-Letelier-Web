      ...
      <div className="max-w-6xl mx-auto px-6 py-12 grid grid-cols-1 sm:grid-cols-2 gap-10">
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            <p className="font-mono text-emerald-500 font-bold text-sm">
              &lt;/&gt;
            </p>
            <h3 className="text-white font-bold text-base">Oscar Letelier</h3>
            <p className="text-zinc-500 text-xs leading-relaxed max-w-xs">
              Desarrollador Full-Stack con foco en Java y React. Siempre abierto
              a nuevos proyectos y colaboraciones.
            </p>
          </div>
            ...
            {navigationItems.map((item) => (
              <li key={item.href}>
                ...
                >
                  <item.icon
                    size={12}
                    className="shrink-0 group-hover:text-emerald-400"
                  />
                  {item.label}
                </a>
              </li>
      ...
      <div className="border-t border-zinc-800/60 px-6 py-4">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="font-mono text-[11px] text-zinc-600">
            © {currentYear}{" "}
            <span className="text-emerald-500">Oscar Letelier</span> — React +
            Vite + Tailwind CSS
          </p>
